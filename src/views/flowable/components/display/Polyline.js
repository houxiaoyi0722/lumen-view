...
function Anchor(uuid, type, x, y) {
  ...
  this.y = y;
  this.type =
    type == ANCHOR_TYPE.middle ? ANCHOR_TYPE.middle : ANCHOR_TYPE.main;
}
Anchor.prototype = {
  uuid: null,
  x: 0,
...
Polyline.prototype = {
  ...
  getLineLength: function (i) {
    return Math.sqrt(
      Math.pow(this.getLineLengthX(i), 2) + Math.pow(this.getLineLengthY(i), 2)
    );
  },
  ...
  getAnchorsCount: function (type) {
    if (!type) return this.anchors.length;
    else {
      let count = 0;
  ...
  rebuildPath: function () {
    ...
    for (let i = 0; i < this.getAnchorsCount(); i++) {
      ...
      if (i > 0 && i < this.getAnchorsCount() - 1) {
        ...
        this.isDefaultConditionAvailable =
          this.isDefaultConditionAvailable || (i == 1 && AO > 10);
